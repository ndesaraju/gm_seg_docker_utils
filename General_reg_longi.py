...
def Syn(arg,file_handl):
	...
	for argum in arg:
		...
		if not os.path.exists(output_path):
			...
			#print(output_path)
			os.makedirs(output_path+'warped')
		...
		for i in range(len(files)):
			...
			cmd1=['WarpImageMultiTransform',str(dim),files2[i],output_path+'warped/'+os.path.basename(files2[i]).split('.')[0]+mse+mse2+'.nii.gz',output_path+'warp'+os.path.basename(files[i]).split('.')[0]+'1Warp.nii.gz',output_path+'warp'+os.path.basename(files[i]).split('.')[0]+'0GenericAffine.mat','-R',static_path]
