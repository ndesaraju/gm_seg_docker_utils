...
def rounds(x):
    if x%1>=.75:
        ...
        return int(x)
def create_prob_seg_iteration3(template_grays,templates,image):
        a=nb.load(image)
        adat_raw=a.get_data()
        ...
        for i in template_grays:
            print(c.get_mse(os.path.basename(i)),i)
            data_dict[c.get_mse(os.path.basename(i))]=i
        ...
        for i in templates:
            ...
            try:
                ...
                continue
            try:
                print(data_dict[c.get_mse(os.path.basename(i))],i)
                fg=nb.load(data_dict[c.get_mse(os.path.basename(i))]).get_data()
            except:
                print(sys.exc_info())
                continue
...
output_from_midnight=glob('/data/henry10-w/jjuwono/test/ms*_*')
output_from_midnight=[os.path.basename(x) for x in output_from_midnight if os.path.basename(x) not in exempt]
for goober in output_from_midnight:
    if 'ms3739retest_PHILIPS' in goober:
        exemption_flag=True
    if exemption_flag:
    	glob_path=glob_path+glob('/data/henry4/jjuwono/new_GM_method/{}/raw_im.nii.gz'.format(goober))
naming='test_retest'
...
for static in glob_path:
    ...
    try:
        os.remove('/data/henry10-w/jjuwono/test/'+mse+'/warped/synslice_avggmsegs.nii.gz')
    except:
        pass

    output_path='/data/henry4/jjuwono/data/henry4/jjuwono/'+naming+'/'+mse+'/'
...
###run process to grab distributions##
    ...
    template_grays=glob('/data/henry10-w/jjuwono/test/'+mse+'/warped/*ms*')
    templates=glob('/data/henry4/jjuwono/data/henry4/jjuwono/'+naming+'/'+mse+'/warped1/*ms*')
    fgs,distributions,a,adat,adat_raw=create_prob_seg_iteration3(template_grays,templates,static) 
    avgim('/data/henry10-w/jjuwono/test/'+mse+'/warped/')
###run process to fit lines###

    
    aff=nb.load(static)
    adat_raw=nb.load(static).get_data()
    first_tmap=nb.load(glob('/data/henry4/jjuwono/new_GM_method/'+mse+'/t_map.nii.gz')[0]).get_data()
    adat_z=z_score(adat,mapt=first_tmap)
    print(adat.dtype)
    mask=nb.load(glob('/data/henry10-w/jjuwono/test/'+mse+'/warped/syn*')[0]).get_data()
    ...
    try:
        os.mkdir('/data/henry4/jjuwono/new_GM_method/'+mse)
    except:
        pass
    ...
    nb.save(nb.Nifti1Image(t_map,aff.affine),'/data/henry4/jjuwono/new_GM_method/'+mse+'/rereg_t_map.nii.gz')
    #nb.save(nb.Nifti1Image(color_im,aff.affine),'/data/henry4/jjuwono/new_GM_method/'+mse+'/color_im.nii.gz')
    nb.save(nb.Nifti1Image(original_line_fit,aff.affine),'/data/henry4/jjuwono/new_GM_method/'+mse+'/rereg_original_line_fit.nii.gz')
    pass_on.append(mse)
